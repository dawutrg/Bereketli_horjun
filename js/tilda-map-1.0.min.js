  ...
  Array.prototype.forEach.call(gmaps, function (gmap) {
    ...
    function t_map__attachInfoMessage(marker, descr, title, isOpen) {
      ...
      var baloonTitle,
        ...
        baloonContent = [
          ...
          .filter(Boolean)
          .join("<br>"),
        infowindow = new google.maps.InfoWindow({ content: baloonContent });
    ...
    if (arMarkers.length > 1) {
      ...
      var listener = google.maps.event.addListener(map, "idle", function () {
        (map.getZoom() > parseInt(mapZoomAttr, 10) || 0 === map.getZoom()) &&
    ...
    google.maps.event.addDomListener(window, "resize", function () {
      ...
      google.maps.event.trigger(map, "resize"),
        ...
        arMarkers.length > 0 &&
          (map.fitBounds(bounds),
          zoom > 0 &&
            (map.getZoom() > zoom || 0 == map.getZoom()) &&
            map.setZoom(zoom));
    }),
...
function t_handleYandexApiReady(recid) {
  ...
  Array.prototype.forEach.call(ymapsArr, function (ymap) {
    ...
    var myLatLng = arMarkers.length > 0 && [
      ...
      eventsPane,
      eventsPaneEl = map.panes.get("events").getElement(),
      mobilePanelText = {
    ...
    function getIconImageOffset(marker) {
      return marker.url && marker.offset
        ? marker.offset.replace(/\s/g, "").split(",").map(Number)
        : marker.url
        ? [0, -48]
        : void 0;
    }
    Array.prototype.forEach.call(
      Object.keys(mobilePanelStyles),
      function (name) {
        ...
        ymaps.domEvent.manager.add(eventsPaneEl, "touchmove", function (event) {
          1 === event.get("touches").length &&
            ...
            (eventsPaneEl.textContent =
              mobilePanelText[window.browserLang] || mobilePanelText.EN),
    ...
    var placemarks = arMarkers.map(function (marker) {
        ...
        return new ymaps.Placemark(
          myLatlng,
          { hintContent: marker.title, balloonContent: baloonContent },
          {
    ...
    if (mapModeAttr) {
      var groundPaneElementChild = ymap.querySelector(
        'ymaps[class$="ground-pane" i] > ymaps'
      );
      if (
        (groundPaneElementChild &&
          (groundPaneElementChild.style.filter =
            "bw_light" === mapModeAttr
              ? "grayscale(1)"
              : "grayscale(1) invert(1)"),
        mapColorAttr)
      ) {
        var groundPaneElement = ymap.querySelector(
            'ymaps[class$="ground-pane" i]'
          ),
    ...
    arMarkers.length > 1
      ? (map
          .setBounds(collection.getBounds(), { checkZoomRange: !0 })
          .then(function () {
            zoom > 0 &&
              (0 == map.getZoom() || map.getZoom() > zoom) &&
              map.setZoom(zoom);
          }),
        zoom > 0 &&
          (0 == map.getZoom() || map.getZoom() > zoom) &&
...
function t_map_hexToHsl(H) {
  ...
  return (
    ...
    [
      h,
      (s = +(
        100 * (s = 0 === delta ? 0 : delta / (1 - Math.abs(2 * l - 1)))
      ).toFixed(1)),
      (l = +(100 * l).toFixed(1)),
    ]
  );
